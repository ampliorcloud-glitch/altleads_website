...
const SECTION_PRESETS = {
  ...
  differentiator: {
    nodeSpread: 0,
    lineOpacity: 1,
    blobScale: 0.85,       // much smaller so it doesn't overlap text
    blobOpacity: 1,
    blobPositionX: 2.2,    // push it further right
    cameraZ: 8.5,          // pull camera back slightly
...
// Plain mutable state for R3F frame polling (replaced zustand)
// ✂️ ponytail: yagni - zustand is overkill for simple non-reactive frame-polled values
const useBlobState = {
  ...SECTION_PRESETS.hero,
  activeSection: 'hero',
  osProgress: 0,
  targetSubColor: '#2196F3',
  getState: () => useBlobState,
  setSection: (name) => {
    const preset = SECTION_PRESETS[name]
    if (!preset) return
    Object.assign(useBlobState, { activeSection: name, ...preset })
  },
  setOsProgress: (val) => { useBlobState.osProgress = val },
  setTargetSubColor: (color) => { useBlobState.targetSubColor = color }
}
